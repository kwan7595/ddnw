...
def model_expected_profiling(model):
    n_macs = 0
    n_params = 0
    ...
    # to compute the flops for conv1 we need to know how many input nodes in layer 1 have an output.
    # this is the effective number of output channels for conv1
    layer1_n_macs, layer1_n_params, input_with_output, _ = model.layers[0].expected_profiling(
        spatial
    )
    ...
    for i, layer in enumerate(model.layers):
        if i != 0:
            spatial = spatial // 2  # stride 2 for all blocks >= 1
            layer_n_macs, layer_n_params, _, output_with_input = layer.expected_profiling(
                spatial
            )
            n_macs += layer_n_macs
            n_params += layer_n_params
